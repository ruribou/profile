          ...
          <div className="flex-shrink-0 animate-reveal-up">
            <div className="relative">
              <Avatar className="w-40 h-40 md:w-48 md:h-48 shadow-2xl shadow-purple-200/30 dark:shadow-purple-900/30">
                <AvatarImage src={data.avatar_url} alt={data.full_name} className="object-cover" />
          ...
          <div className="flex-1 text-center md:text-left space-y-5">
            <div className="animate-reveal-up delay-100">
              <div className="flex items-center justify-center md:justify-start gap-2 mb-2">
                ...
                <span className="text-xs font-mono text-gray-500 dark:text-gray-400 tracking-wider uppercase">{data.title}</span>
              </div>
              <h1 className="text-5xl md:text-7xl font-extrabold tracking-tight text-gray-800 dark:text-gray-50 leading-tight">
                {data.name}
              </h1>
              <p className="text-gray-500 dark:text-gray-400 text-sm tracking-widest uppercase mt-1">{data.full_name}</p>
            </div>
            ...
            <div className="flex flex-col items-center md:items-start gap-3 animate-reveal-up delay-200">
              ...
              <div className="flex gap-2 flex-wrap justify-center md:justify-start">
                {(data.badges as string[]).map((badge) => (
                  <span
                    key={badge}
                    className="text-xs font-semibold px-3 py-1.5 rounded-full bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 border border-blue-100 dark:border-blue-500/20"
                  >
                    {badge}
                  </span>
                ))}
